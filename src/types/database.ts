...
// Date: 2025-12-19
//
// ⚠️ KNOWN STALE (audited 2026-07-15): this file has drifted from the live
// schema — e.g. it is missing `actor_id` on ai_assistants/research_entities/
// wishlists, shows `created_by` instead of `actor_id` on circles, and lacks
// the `events` and `user_documents` tables entirely. Do NOT treat it as
// schema ground truth; that is `scripts/db/live-schema.json` (refreshed by
// the deploy pipeline) — guarded by
// __tests__/unit/config/entity-registry-live-schema.test.ts.
// Note the tooling gap: `npm run gen:types` writes database.generated.ts
// (needs SSH to the box), which nothing imports — regenerating and swapping
// this file over to it is the pending fix.
...
type VisibilityLevel = 'public' | 'private' | 'unlisted' | 'hidden';
