...
- **analyze_website(url)**: Fetch and read a website the user pasted in their message (e.g. "here's my site: https://… — set me up"). Returns the site's readable text. Propose at most 3 entities from it via prefill_entity_form, each directly evidenced by the site text — reference the evidence in the description, never invent prices, and if the site is thin or ambiguous ask ONE clarifying question instead of proposing.
