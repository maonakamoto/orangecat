  ...
  return (
    <div className={cn('flex items-center gap-3', className)}>
      ...
      <div className="flex flex-col items-center">
        ...
        <div
          className={cn(
            'w-0.5 bg-surface-raised flex-shrink-0',
            depth > 1 ? 'h-4 -mt-2 mb-2' : 'h-2 mb-2'
          )}
        />

        {/* Horizontal connector (only for nested replies) */}
        {depth > 1 && <div className="w-6 h-0.5 bg-surface-raised flex-shrink-0" />}
      </div>
  ...
  return (
    <div className={cn('flex items-start', className)}>
      ...
      <div className="flex flex-col items-center w-8 flex-shrink-0">
        ...
        {depth > 0 && <div className="w-0.5 bg-surface-raised h-2" />}

        {/* Thread node */}
        <div className="w-2 h-2 bg-fg-secondary rounded-full flex-shrink-0" />

        {/* Line below current post (if has children) */}
        {hasChildren && !isLastInThread && (
          <div className="w-0.5 bg-surface-raised flex-1 min-h-4" />
        )}
      </div>
