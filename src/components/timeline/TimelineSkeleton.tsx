  ...
  return (
    <div className="space-y-0">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="animate-pulse border-b border-subtle bg-surface-page">
          <div className={`p-4 ${compact ? 'p-3' : 'p-4'}`}>
            <div className="flex items-start space-x-3">
              {/* Avatar skeleton */}
              <div
                className={`flex-shrink-0 rounded-md bg-surface-raised ${compact ? 'h-8 w-8' : 'h-10 w-10'}`}
              />
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="h-3 bg-surface-raised rounded w-8" />
                </div>
              </div>
            </div>
          </div>
