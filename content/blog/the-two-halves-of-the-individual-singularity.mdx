...
That is the problem [Cockpit](https://cockpitapp.vercel.app) exists to solve. Local execution where the work happens. Remote command from anywhere. Per-project autonomy levels — manual, queue, beacon, continuous, mission — so the operator dials in how much initiative the system takes per surface. Truthful status, legible handoffs, a real audit log.
